...
function Mini.grep(cwd, pattern)
  ...
  if ok and mini_pick.builtin and mini_pick.builtin.grep_live then
    ...
    if pattern and pattern ~= "" then opts.input = vim.split(pattern, "") end
...
function Mini.live_grep(host, mount_path, path, callback)
  ...
  local function open_grep_result(item)
    ...
    vim.schedule(function()
      local open_ok, err = pcall(function()
        ...
        if col then vim.api.nvim_win_set_cursor(0, { tonumber(lnum), tonumber(col) - 1 }) end
      end)

      if not open_ok then vim.notify("Error opening file: " .. tostring(err), vim.log.levels.ERROR) end
    end)
  end
  ...
  local source = {
    ...
    match = function(_, _, query)
      ...
      if not query or query == "" then return mini_pick.set_picker_items({}, set_items_opts) end
...
function Mini.live_find(host, mount_path, path, callback)
  ...
  local items = {}
  for _, line in ipairs(output) do
    if line and line ~= "" then table.insert(items, line) end
  end
