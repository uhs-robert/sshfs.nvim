...
local function cleanup_scratch_resources(scratch_buf, augroup)
  vim.defer_fn(function()
    if vim.api.nvim_buf_is_valid(scratch_buf) then pcall(vim.api.nvim_buf_delete, scratch_buf, { force = true }) end
...
function Ranger.explore_files(cwd)
  local ok, ranger = pcall(require, "ranger-nvim")
  if ok and ranger.open then return open_ranger_nvim(cwd) end
