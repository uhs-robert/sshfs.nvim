...
local function get_integration(module_path)
  if not INTEGRATION_CACHE[module_path] then INTEGRATION_CACHE[module_path] = require(module_path) end
  ...
  if preferred and preferred ~= "auto" then
    for _, picker in ipairs(picker_registry) do
      if picker.name == preferred then
        ...
        if integration[picker.method](unpack(args)) then return true, picker.name end
        break
      end
    end
  end
...
function Picker.browse_remote_files(opts)
  ...
  if not config or not active_connection or not target_dir then return end
  ...
  if not success then vim.notify("Failed to open " .. picker_name .. ". Please open manually.", vim.log.levels.WARN) end
end
...
function Picker.grep_remote_files(pattern, opts)
  ...
  if pattern and pattern ~= "" then vim.fn.setreg("/", pattern) end
  vim.notify(
  ...
  if preferred and preferred ~= "auto" then
    for _, picker in ipairs(LIVE_REMOTE_GREP_PICKERS) do
      if picker.name == preferred then
        ...
        if success then return true, picker.name end
        break
      end
    end
  end
