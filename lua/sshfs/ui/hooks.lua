...
local function normalize_action(action)
  if action == nil or action == "none" then return nil end
  ...
  if not allowed[preset] then preset = "find" end
...
local function find_connection_by_mount(mount_dir)
  ...
  for _, conn in ipairs(MountPoint.list_active()) do
    if conn.mount_path == mount_dir then return conn end
  end
  return nil
end
...
local function run_preset_action(preset, mount_dir, config)
  ...
  if preset == "live_grep" or preset == "live_find" then
    ...
    if not conn then
      vim.notify(
        "No connection found for on_mount action: "
          .. mount_dir
...
function Hooks.on_mount(mount_dir, host, remote_path, config)
  ...
  if auto_change_to_dir then vim.cmd("tcd " .. vim.fn.fnameescape(mount_dir)) end
  ...
  if type(action) == "function" then
    ...
    if not ok then vim.notify("on_mount callback failed: " .. err, vim.log.levels.ERROR) end
    return
  end
