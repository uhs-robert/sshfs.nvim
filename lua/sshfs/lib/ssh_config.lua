...
local function get_last_modified(files)
	...
	for _, file in ipairs(files) do
		...
		if stat and stat.mtime.sec > modified_time then
	...
	for line in output:gmatch("[^\r\n]+") do
		...
		if key and value then
			...
			if config[normalized_key] then
				if type(config[normalized_key]) ~= "table" then
					config[normalized_key] = { config[normalized_key] }
				end
...
function SSHConfig.get_host_config(hostname)
	if CACHE.resolved_configs[hostname] then
		return CACHE.resolved_configs[hostname], nil
	end
