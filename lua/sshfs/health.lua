...
local function check_system_dependencies()
  ...
  if command_exists("ssh") then
    ...
    health.error("ssh is not installed", "Install OpenSSH client: 'sudo apt install openssh-client' (Debian/Ubuntu)")
  end
  ...
  if command_exists("mount") then
    ...
    health.warn("mount command is not available", "Mount detection may not work properly. Install util-linux package.")
  end
  ...
  for _, method in ipairs(unmount_methods) do
    if command_exists(method.cmd) then table.insert(available_unmount, method.desc) end
  end
...
local function check_ssh_config()
  ...
  if #found_configs > 0 then health.ok("Found SSH config files: " .. table.concat(found_configs, ", ")) end
...
local function check_mount_config()
  ...
  if is_writable(base_dir) then
    ...
    health.error("Mount base directory is not writable: " .. base_dir, "Fix permissions with: chmod u+w " .. expanded)
  end
...
local function check_integrations()
  ...
  for _, integration in ipairs(integrations) do
    if plugin_loaded(integration.plugin) then table.insert(available_pickers, integration.name) end
  end

  -- Check external file managers
  for _, manager in ipairs(file_managers) do
    if command_exists(manager.cmd) then table.insert(available_managers, manager.name) end
  end
...
local function check_configuration()
  ...
  if not (config.global_paths and #config.global_paths > 0) and not (config.host_paths and next(config.host_paths)) then
