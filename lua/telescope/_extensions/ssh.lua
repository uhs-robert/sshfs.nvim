		...
		.new(_, {
			...
			previewer = previewers.new_buffer_previewer({
				...
				define_preview = function(self, entry)
					...
					vim.bo[bufnr].filetype = "sshconfig"
...
local function find_files(opts)
	local connections
	if pcall(require, "ssh") then
		connections = require("ssh.connections")
	else
	...
	local find_command = (function()
		if not current_host or not current_host["Name"] then
			vim.notify("Error: No active SSH connection", vim.log.levels.ERROR)
			return nil
		end

		-- Check if the user provided a custom find command
		...
		-- Determine the best available find command
		if command_exists_on_remote("rg", current_host["Name"]) then
...
local function live_grep(opts)
	...
	if current_host and current_host["Name"] then
		vimgrep_arguments = {
			"ssh",
			current_host["Name"],
			"-C",
		...
		vim.notify("Error: No active SSH connection (current_host is nil)", vim.log.levels.ERROR)
		return
	end
		...
		.new(opts, {
			...
			previewer = previewers.new_buffer_previewer({
				...
				define_preview = function(self, entry, _)
					...
					local has_buftype = entry.bufnr and vim.bo[entry.bufnr].buftype ~= "" or false
					local p
